    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 mx-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Student Project ID *</label>
            <input
              type="text"
              required
              value={studentProjectId}
              onChange={(e) => setStudentProjectId(e.target.value)}
              placeholder="UUID of completed StudentProject"
            ...
            />
            <p className="text-xs text-slate-500 mt-1">Must refer to a completed StudentProject and Student must have completed the curriculum.</p>
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Notes (Admin only)</label>
            <textarea
              value={notes}
              ...
              {filtered.map((cert) => (
                <tr key={cert.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-5 py-4">
                    <div className="font-medium text-slate-900 text-sm">{cert.student.name}</div>
                    <div className="text-xs text-slate-500">{cert.student.studentId}</div>
                  </td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      <a
                        href={`${API_URL}/certificates/verify/${cert.verificationToken}`}
                        target="_blank"
                        rel="noreferrer"
                        className="text-xs bg-slate-100 text-slate-700 px-2 py-1 rounded-lg font-medium hover:bg-slate-200"
                        title="View/Verify"
                      >
                        Verify
                      </a>
